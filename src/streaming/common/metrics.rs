...
impl MetricsManager {
    ...
    ///
    /// # 功能说明
    /// - 如果 `enable_metrics` 为 `false`，直接返回 `None`，不启动监控
    /// - 如果启用监控：
    ///     1️⃣ 克隆当前 `MetricsManager` 实例
    ///     2️⃣ 使用 `tokio::spawn` 启动一个异步任务
    ///     3️⃣ 在任务中创建一个固定间隔的定时器 `interval`
    ///        - 间隔时间为 `DEFAULT_METRICS_PRINT_INTERVAL_SECONDS` 秒
    ///     4️⃣ 循环等待定时器 tick，每到一次 tick 调用 `manager.print_metrics()`
    ///        - 定期打印性能指标（如吞吐量、延迟、丢弃事件数等）
    ///
    /// # 返回值
    /// - `Some(JoinHandle<()>)`：异步任务句柄，可用于停止或管理监控任务
    /// - `None`：如果监控未启用
    ///
    /// # 异步特性
    /// - 函数为 `async`，返回的任务在后台持续运行
    /// - 通过 `tokio::spawn` 实现非阻塞定时打印
